"use client";
// S12 (assessor view) — STATIC LAYOUT MOCK for 1A.
// No video yet: real AV integration is Chunk 1D, gated on the spike verdict.
// Mock capture is local-state only, to demonstrate the capture-loop concept:
// select item → click/hotkey → auto-labelled thumbnail in tray → no modal.
import { useParams } from "next/navigation";
import { useEffect, useMemo, useState } from "react";
import Link from "next/link";
import { PrototypeBanner, PhotoTile } from "@/components/Chrome";
import { jobById, templateById, clientById } from "@/lib/fixtures";
...
export default function LiveRoom() {
  ...
  const activeItem = useMemo(() => {
    if (!tpl || !active) return undefined;
    for (const s of tpl.sections) for (const i of s.items) if (i.key === active) return { s, i };
    return undefined;
  }, [tpl, active]);
  ...
  // Hotkey: C captures when not typing (doc 04 §4.4 concept demo).
  useEffect(() => {
  ...
  if (!job || !tpl) return <div className="p-8">Unknown job.</div>;

  return (
    <div className="min-h-screen bg-slate-900 flex flex-col">
      <PrototypeBanner />
      <header className="px-4 py-2 flex items-center gap-3 text-slate-300 text-sm">
        <Link href={`/jobs/${job.id}`} className="hover:text-white">← {job.jobNumber}</Link>
        <span>{clientById(job.clientId)?.name}</span>
      ...
      <div className="flex-1 grid grid-cols-[3fr_2fr] gap-2 px-2 pb-2 min-h-0">
        ...
        <div className="rounded-xl bg-slate-100 overflow-y-auto p-3">
          <div className="text-xs text-slate-500 mb-2">
            {tpl.name} v{tpl.version} — tap an item to make it active, then Capture (or press C).
          </div>
          {tpl.sections.map((s) => (
